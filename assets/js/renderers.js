            ...
            return `
                <div class="gallery-item ${item.size === 'large' ? 'large' : ''}" data-category="${item.category}">
                    <a href="${basePath}${item.image}" class="glightbox" data-gallery="home-gallery" data-title="${item.category}" data-description="${item.alt}" style="display: block; width: 100%; height: 100%;">
                        <video src="${basePath}${item.image}" aria-label="${item.alt}" autoplay muted loop playsinline preload="metadata" style="width: 100%; height: 100%; object-fit: cover; ${item.rotate ? `transform: rotate(${item.rotate}) scale(1.5);` : ''}"></video>
                    </a>
                </div>
            `;
        }
...
export function renderAboutSection(data, container, basePath = "") {
    ...
    const html = `
        <div class="about-left">
            <img src="${basePath}${data.about.image || 'assets/images/wedding-photo-10.webp'}" alt="Bridal makeup look" width="720" height="820" loading="lazy">
        </div>
...
export function renderAboutPage(data, container, basePath = "") {
    ...
    <div class="about-page-content">
        ...
        <div class="about-image">
            <img src="${basePath}assets/images/wedding-photo-11.webp" alt="RR Studio Photographer at Work" loading="lazy">
        </div>
    </div>
    ...
    const itemsHtml = `
        <div class="masonry-gallery">
            ${data.gallery.map(item => {
                if (item.type === 'video') {
                    return `
                        <div class="masonry-item ${item.category} video-item">
                            <a href="${basePath}${item.image}" class="glightbox" data-gallery="full-gallery" data-title="${item.category}" data-description="${item.alt}" style="display: block;">
                                <video src="${basePath}${item.image}" aria-label="${item.alt}" autoplay muted loop playsinline preload="metadata" style="width: 100%; height: 100%; object-fit: cover;"></video>
                            </a>
                        </div>
                    `;
                }
